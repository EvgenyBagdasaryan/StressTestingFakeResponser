...
public class NetworkClient implements KafkaClient {
    ...
    public NetworkClient(MetadataUpdater metadataUpdater,
        ...
        this.log = LogManager.getLogger(NetworkClient.class);//logContext.logger(NetworkClient.class);
    ...
    public void disconnect(String nodeId) {
        ...
        if (log.isTraceEnabled()) {
            log.trace("Manually disconnected from " + nodeId + ". Aborted in-flight requests: " + inFlightRequests);
        }
    }
    ...
    private void cancelInFlightRequests(String nodeId, long now, Collection<ClientResponse> responses) {
        ...
        for (InFlightRequest request : inFlightRequests) {
            log.trace("Cancelled request " + request.header.apiKey() + " " + request.request + " with correlation id " + request.header.correlationId() + " due to node " + nodeId + " being disconnected");
        ...
        try {
            ...
            if (versionInfo == null) {
                ...
                if (discoverBrokerVersions && log.isTraceEnabled())
                    log.trace("No version information found when sending " + clientRequest.apiKey() + " with correlation id " + clientRequest.correlationId() + " to node " + nodeId + ". " +
            ...
            log.debug("Version mismatch when attempting to send " + builder + " with correlation id " + clientRequest.correlationId() + " to " + clientRequest.destination() +  " : " +  unsupportedVersionException);
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now, AbstractRequest request) {
        ...
        if (log.isDebugEnabled()) {
            log.trace("Sending " + clientRequest.apiKey() + " request with header " + header + " and timeout " + clientRequest.requestTimeoutMs() + " to node " + destination + ": " + request);
        }
    ...
    public Node leastLoadedNode(long now) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (canSendRequest(node.idString(), now)) {
                ...
                if (currInflight == 0) {
                    ...
                    log.trace("Found least loaded node " + node + " connected with no in-flight requests");
                    return node;
                ...
                log.trace("Removing node " + node + " from least loaded node selection since it is neither ready " +
                        "for sending or connecting");
            }
        }
        ...
        if (foundReady != null) {
            log.trace("Found least loaded node " + foundReady + " with " + inflight + " inflight requests");
            return foundReady;
        } else if (foundConnecting != null) {
            log.trace("Found least loaded connecting node " + foundConnecting);
            return foundConnecting;
        } else if (foundCanConnect != null) {
            log.trace("Found least loaded node " + foundCanConnect + " with no active connection");
    ...
    private void processDisconnection(List<ClientResponse> responses,
        ...
        switch (disconnectState.state()) {
            case AUTHENTICATION_FAILED:
                ...
                log.error("Connection to node " + nodeId + " (" + disconnectState.remoteAddress() + ") failed authentication due to: " + exception.getMessage());
                break;
            case AUTHENTICATE:
                ...
                break;
            case NOT_CONNECTED:
                log.warn("Connection to node " + nodeId + " (" + disconnectState.remoteAddress() + ") could not be established. Broker may not be available.");
                break;
            default:
    ...
    private void handleTimedOutRequests(List<ClientResponse> responses, long now) {
        ...
        for (String nodeId : nodeIds) {
            ...
            log.debug("Disconnecting from node " + nodeId + " due to request timeout.");
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            if (log.isDebugEnabled()) {
                log.trace("Received " + req.header.apiKey() + " response from node " + req.destination + " for request with header " + req.header + " " + response);
            }
    ...
    private void handleApiVersionsResponse(List<ClientResponse> responses,
        ...
        if (apiVersionsResponse.data().errorCode() != Errors.NONE.code()) {
            if (req.request.version() == 0 || apiVersionsResponse.data().errorCode() != Errors.UNSUPPORTED_VERSION.code()) {
                log.warn("Received error " + Errors.forCode(apiVersionsResponse.data().errorCode()) + " from node " + node + " when making an ApiVersionsRequest with correlation id " + req.header.correlationId() + ". Disconnecting.");
        ...
        log.debug("Node " + node + " has finalized features epoch: " + apiVersionsResponse.data().finalizedFeaturesEpoch() + ", finalized features: " + apiVersionsResponse.data().finalizedFeatures() + ", supported features: " + apiVersionsResponse.data().supportedFeatures() + ", API versions: " +  nodeVersionInfo);
    }
    ...
    private void handleDisconnections(List<ClientResponse> responses, long now) {
        for (Map.Entry<String, ChannelState> entry : this.selector.disconnected().entrySet()) {
            ...
            log.debug("Node " + node + " disconnected.");
    ...
    private void handleConnections() {
        for (String node : this.selector.connected()) {
            ...
            if (discoverBrokerVersions) {
                ...
                log.debug("Completed connection to node " + node + ". Fetching API versions.");
            } else {
                this.connectionStates.ready(node);
                log.debug("Completed connection to node " + node + ". Ready.");
            }
        }
    }
    ...
    private void initiateConnect(Node node, long now) {
        ...
        try {
            ...
            log.debug("Initiating connection to node " + node + " using address " + address);
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        public void handleSuccessfulResponse(RequestHeader requestHeader, long now, MetadataResponse response) {
            ...
            if (!missingListenerPartitions.isEmpty()) {
                ...
                log.warn(count + " partitions have leader brokers without a matching listener, including " + missingListenerPartitions.subList(0, Math.min(10, count)));
            }
        ...
        private long maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId, now)) {
                ...
                log.trace("Sending metadata request " + metadataRequest + " to node " +  node);
            ...
            if (connectionStates.canConnect(nodeConnectionId, now)) {
                ...
                log.debug("Initialize connection to node " + node + " for sending metadata request");
                initiateConnect(node, now);
                return reconnectBackoffMs;
            }
